...
// ---------- Compras ----------
/**
 * Registra uma compra por lote. O lote de destino pode ser criado na hora
 * ("novo") ou já existir ("existente") — nos dois casos a quantidade do lote
 * é incrementada e o custo por cabeça é recalculado. Quando o lote já tinha
...
/**
 * Exclui uma compra. A quantidade do lote vinculado volta atrás (o que essa
 * compra somou é subtraído). O custo por cabeça do lote NÃO é revertido —
 * depois de outras compras/vendas nesse meio tempo, "desfazer" a média
 * ponderada com exatidão deixaria de ser confiável, então preferimos manter
 * o último custo calculado a arriscar um número que pareça exato mas não é.
 */
...
export async function createSaleAction(formData: FormData) {
  ...
  if (saleKind === "lote") {
    ...
    const lot = animal.lotId
      ...
      : null;
    const costBasis = lot?.costPerHead ?? null;
    const profit = costBasis != null ? totalValue - costBasis : null;
